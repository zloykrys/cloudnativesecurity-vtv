...
* **Target Application:** SUSE Application Collection MariaDB 
...
kubectl create namespace mariadb-secure
...
1. **Bootstrapping Layer:** A Kubernetes Job securely injects the SUSE AppCo public key into NeuVector's internal Sigstore engine using parameterized references to the pre-created cluster secret.
...
3. **Application Layer:** Deploys the MariaDB workload, referencing the pre-created `imagePullSecret` to safely authenticate against the AppCo registry.

## Verification Steps
1. Execute the commands outlined in the **Pre-Requisite Cluster Instructions** section above.
2. Push the deployment, bootstrap job, and security configurations to your Git repo monitored by your GitOps engine (e.g., Fleet, ArgoCD, or Flux CD).
3. Verify that the signed MariaDB deployment spins up successfully.
4. Modify the `mariadb-deployment.yaml` to use an unsigned image (e.g., `docker.io/library/mariadb:latest`) and push the update.
5. Verify that the deployment is instantly rejected.
